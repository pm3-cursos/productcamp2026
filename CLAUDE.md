...
- **Dados estruturados:** a home tem JSON-LD `schema.org/Event` com `offers` (preço, lote e `priceValidUntil`); a `/para-empresas/` tem `Event` + `FAQPage`. **As ofertas mudam a cada virada de lote.** O calendário e os preços do Passaporte vivem em `assets/js/pcamp-countdown-lotes.js` (`LOTES`), e o script aplica sozinho o lote ativo no badge, no preço e na oferta do JSON-LD. Separe as duas coisas: **para o visitante, a virada é automática na data, sem deploy; para SEO, o deploy de acompanhamento continua obrigatório.** Sem ele, o HTML bruto — que é o que crawler sem JS, crawler de IA e a primeira passada de indexação do Google leem — segue anunciando o preço do lote anterior, com `priceValidUntil` já vencido; e o `llms.txt`, que nenhum script alcança, também. Preço na marcação que não bate com o da página e o do checkout custa rich result. No mesmo deploy, atualize: o `<span data-pcamp-lote-badge>` e o `<strong data-pcamp-preco>` do Passaporte, o `offers[0]` de `#pcamp-event-jsonld` (nome, `price` e `priceValidUntil`), o `llms.txt` e o `lastmod` da home no `sitemap.xml`. O card VIP continua 100% manual. Se uma página tem `FAQPage`, o JSON-LD precisa bater com as perguntas visíveis.
